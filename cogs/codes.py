...
class Execution(commands.Cog):
    ...
    async def __execute_code(self, ctx, lang, code: Optional[str]):
        ...
        if code.startswith("-v") or code.startswith("--version"):
            await ctx.send(embed=Embed(title=f"Version para {lang['command']}", color=int(9549258)).set_thumbnail(url=lang["icon"]).add_field(name="\uFEFF", value=f"> {lang['version']}"))
    ...
    async def run(self, ctx, *, code: Optional[str]):
        ...
        if str(ctx.invoked_with) == "run" and code == "list" or code == "--list":
            embed = Embed(title="Lista de lenguages", color=int(9549258))
            listaDeLenguages = ""   
            c = 0
    ...
    def resize_output_for_embed(output, embed, token):
        ...
        if len(output) > 300 or output.count("\n") > 10:
            embed.description = f"Salida demasiado grande - [Salida completa](https://maubot.maucode.com/compilador?{token})"
            ...
            embed.description = f"Edite este código en un IDE en línea - [Aqui](https://maubot.maucode.com/compilador?{token})"
